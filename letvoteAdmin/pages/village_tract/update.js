...
const VillageTractUpdate = (props) => {
    ...
    const onFinish = async(values) => {
        let result = await api().put(`village_tract/${values.id}`, values).then(res => {
            if(res && res.data.status === 200){
                noti('success', "village_tract updated successful.")
    ...
    return (
        state && <Card title="Edit village_tract">
            <Form
                onFinish={onFinish}
...
export default withRouter(VillageTractUpdate);