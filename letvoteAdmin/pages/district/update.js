...
const DistrictUpdate = (props) => {
    ...
    const onFinish = async(values) => {
        let result = await api().put(`district/${values.id}`, values).then(res => {
            if(res && res.data.status === 200){
                noti('success', "District updated successful.")
    ...
    return (
        state && <Card title="Edit District">
            <Form
                onFinish={onFinish}
...
export default withRouter(DistrictUpdate);